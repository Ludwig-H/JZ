...
\begin{frame}{Outline}
  ...
  % beamer inserts between the entries does not stretch. The \vspace* sets
  % the vertical position to the one measured on the French version's
...
\begin{frame}{Going deep}
  \vspace*{2.06pt}% calibration inherited from the French version
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}{Transformers, self-supervision and foundation models}
  ...
  \vspace*{-1.24pt}% calibration inherited from the French version
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \end{itemize}
    \end{column}
    % Figure placed as in the French version: left edge flush with the column
    % (no \centering here — \legende centers itself) and width
    % 1.0038 times the column, i.e. 154.9 pt.
    \begin{column}{0.41\textwidth}
      \vspace{1.65ex}
      \noindent\includegraphics[width=1.0038\textwidth]{imgs/article/fig7_modele_fondation_geospatial}
