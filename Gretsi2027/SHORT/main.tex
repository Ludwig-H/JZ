...
\begin{frame}{Sommaire}
  ...
  % beamer insère entre les entrées ne s'étirent pas.
...
\begin{frame}{L'apport du contexte spatial}
  ...
  \end{itemize}
  % ATTENTION — défaut hérité du PDF d'origine : `haralick1985image` n'est PAS
  ...
  % d'une référence. Conservé tel quel pour ne pas modifier le contenu de la
  % planche sans arbitrage. Pour corriger : soit retirer la clé de la ligne
  % ci-dessous (la planche ne la cite nulle part ailleurs), soit réintroduire
  % l'entrée dans references.bib — mais cela décalerait tous les numéros [n]
  % suivants. La version anglaise, elle, a choisi de retirer la clé.
